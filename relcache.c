...
static int comp_sys_param(relParam* cpar, relParam* par){
	...
	// if (comp_single_param_val(par->rinp,cpar->rinp)) return 1;
	// non-Kerr mods
	if (comp_single_param_val(par->def_par,cpar->def_par)) return 1;  // non-Kerr mods
	if (comp_single_param_val(par->def_par_unscaled,cpar->def_par_unscaled)) return 1;
	if (comp_single_param_val(par->rin,cpar->rin)) return 1;
	if (comp_single_param_val(par->rinp,cpar->rinp)) return 1;
	if (comp_single_param_val(par->r_ring,cpar->r_ring)) return 1;
	if (comp_single_param_val(par->r_ring_out,cpar->r_ring_out)) return 1;

	if (par->def_par_type != cpar->def_par_type) return 1;
	if (par->mdot_type != cpar->mdot_type) return 1;

	// a twice broken power-law
	if (comp_single_param_val(par->emis3,cpar->emis3)) return 1;
	if (comp_single_param_val(par->rbr2,cpar->rbr2)) return 1;
...
void set_cached_rel_param(relParam* par, relParam** ca_rel_param, int* status){
	...
	// non-Kerr mods
	(*ca_rel_param)->def_par = par->def_par;
	(*ca_rel_param)->def_par_unscaled = par->def_par_unscaled;
	(*ca_rel_param)->def_par_type = par->def_par_type;
	(*ca_rel_param)->mdot_type = par->mdot_type;

	(*ca_rel_param)->rinp = par->rinp;
	(*ca_rel_param)->r_ring = par->r_ring;
	(*ca_rel_param)->r_ring_out = par->r_ring_out;
	// a twice broken-power-law
